/* Set Digital Output for i7k/i87k modules via serial port.
...
int main(int argc, char **argv)
{
	...
	if (module_name[0]=='7') {
		...
		wBuf[6] = 0;	              		//debug
		RetValue = DigitalOut(wBuf,fBuf,szSend,szReceive);
		//dwValue = wBuf[5];
	}
	else {
		...
		dwBuf[7] = slot;            		//slot
		if (module_name[1]=='7')
			RetValue=DigitalOut_87K(dwBuf,fBuf,szSend,szReceive);
		else
			RetValue=DigitalOut_8K(dwBuf,fBuf,szSend,szReceive);
		//dwValue = wBuf[5];
	}
	if (RetValue) {
		printf("Module at COM%d Address %d DO error %d!!!\n",comport,addr,RetValue);
		return FAILURE;
	}
...
void usage(void)
{
	...
	printf("function : setexdo\n");
	printf("Set digital output value to a module\n");
	printf("Usage: setexdo slot 1 data\n");
	printf("       setexdo slot comport data baudrate address\n");
	printf("Example 1:setexdo 2 1 55\n");
	printf("Set the dec digital output value to the module at slot 2\n");
	printf("Example 2:setexdo 0 3 55 9600 2\n");
	printf("Set the dec digital output value to the module at COM3\n");
}

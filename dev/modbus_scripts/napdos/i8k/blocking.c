/* blocking demo for i-8144 module.
 ...
 *  Returned:  void
 */
	sio_set_timer(fd_m, 0, 10);	               /* set timeout for 1 sec */
	sio_set_timer(fd_s, 0, 10);	               /* set timeout for 1 sec */
  ...
  r = read(fd_s, rbuf, sizeof(rbuf));
  
  printf("read %d byte --> ",r);
  for(temp=0;temp<r;temp++) printf("%c",rbuf[temp]);
  printf("\n");

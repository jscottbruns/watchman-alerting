/* Get Analog Input for i7k/i87k modules via serial port.
...
int main(int argc, char **argv)
{
	...
	if (module_name[0]=='7') {
		...
		wBuf[6] = 0;			            //debug
		RetValue = AnalogIn(wBuf,fBuf,szSend,szReceive);
		//dwValue = wBuf[5];
	}
	else {
		...
		dwBuf[7] = slot;	           	//slot
		if (module_name[1]=='7')
			RetValue=AnalogIn_87K(dwBuf,fBuf,szSend,szReceive);
		else
			RetValue=AnalogIn_8K(dwBuf,fBuf,szSend,szReceive);
		//dwValue = wBuf[5];
	}
	...
	if (RetValue) {
		printf("Module at COM%d Address %d AI error %d !!!\n",comport,addr,RetValue);
		return FAILURE;
	}
	printf("%f",fValue);
...
void usage(void)
{
	...
	printf("function : getexai\n");
	printf("Get analog input value from a serial module\n");
	printf("Usage: getexai slot 1 channel\n");
	printf("       getexai slot comport channel baudrate address\n");
	printf("Example 1:getexai 2 1 5\n");
	printf("Get channel 5 analog value from the module at slot 2\n");
	printf("Example 2:getexai 0 3 5 9600 2\n");
	printf("Get channel 5 analog value from the module at COM3\n");
}

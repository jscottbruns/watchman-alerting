...
public class Dio 
{
	public static void main(String[] args) throws java.io.IOException
	{
		...
		if (rev!=0) System.out.println("Open port error code : "+rev);
			...
			while(a[0]!=113) {
				i87kBuf.dwBuf[2] = 0x8060 ;       			//0x8060; //module name
				i87kBuf.dwBuf[5] =i;				            //Digital out
				i87kBuf.dwBuf[7] = 1;			            	//Slot no 1
				rev = comm1.setDigitalOut(i87kBuf);		  //Set Digital Output Value
				...
				i87kBuf.dwBuf[2] = 0x8051 ;		        	//0x8051; //module name
				i87kBuf.dwBuf[7] = 2;		            		//Slot no 2			
				rev = comm1.getDigitalIn(i87kBuf);	  	//Get Digital Input Value
				System.out.println("szSend = "+ i87kBuf.szSend +" szReceive = "+i87kBuf.szReceive);

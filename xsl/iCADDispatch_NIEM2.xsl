...
<xsl:stylesheet
	...
	<xsl:template match="/">
		...
		<WatchmanAlerting>
			...
			<ReportNo><xsl:value-of select="//fhwm:ICadDispatch/ns4:Payload/ns4:ServiceCall/ns4:ServiceCallAugmentation/ns4:LocalIncidentId/ns3:IdentificationID"/></ReportNo>
			...
			<IncidentStatus>
				...
				<xsl:for-each select="//fhwm:ICadDispatch/ns4:Payload/ns4:ServiceCall/ns3:ActivityStatus">
					<xsl:if test="./ns3:StatusText = 'INITIATE'">
						<Create><xsl:value-of select="./ns3:StatusDate/ns3:DateTime"/></Create>
					</xsl:if>				
			...
			<CallType>
				...
				<CallGroup><xsl:value-of select="//fhwm:ICadDispatch/ns4:Payload/ns4:ServiceCall/ns4:ServiceCallAugmentation/ns4:CallSubTypeText"/></CallGroup>
			</CallType>
			<IncidentLocation>
				<BoxArea><xsl:value-of select="substring($BoxArea,5,1)"/>-<xsl:value-of select="substring($BoxArea,6)"/></BoxArea>
				<Station><xsl:value-of select="substring($StationGrid,4)"/></Station>
			...
			<UnitDispatch>
				<xsl:variable name="__Units">
				  <xsl:call-template name="string-replace-all">
				    <xsl:with-param name="text" select="//fhwm:ICadDispatch/ns4:Payload/ns4:ServiceCall/ns3:ActivityReasonText" />
				    <xsl:with-param name="replace" select="'231'" />
				    <xsl:with-param name="by" select="''" />
				  </xsl:call-template>
				</xsl:variable>
				<UnitList><xsl:value-of select="$__Units"/></UnitList>
				<xsl:for-each select="//fhwm:ICadDispatch/ns4:Payload/ns4:ServiceCallAssignedUnit">
					...
					<Assigned><xsl:value-of select="substring($__Unit, 4)"/></Assigned>
				</xsl:for-each>
			</UnitDispatch>
			<IncidentComments>
				<xsl:for-each select="//fhwm:ICadDispatch/ns4:Payload/ns4:ServiceCall/ns4:ServiceCallAugmentation/ns4:Comment">
					...
					<Comment>
						<EntrySequence><xsl:value-of select="./ns3:OrganizationIdentification/ns3:IdentificationID"/></EntrySequence>
